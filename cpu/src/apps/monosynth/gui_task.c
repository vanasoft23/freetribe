...
bool _parse_event(t_gui_event *event);
...
static bool _init(void) {
	...
	if (RING_BUFFER_OK != ring_buffer_init(&g_gui_rbd, g_gui_rbmem,
								 sizeof(g_gui_rbmem[0]), ARRAY_SIZE(g_gui_rbmem))) {
		return false;
	}
...
static void _run(void) {
	...
	if (RING_BUFFER_OK == ring_buffer_get(&g_gui_rbd, &event)) {
		_parse_event(&event);
	}
}
