...
firmware_result_t fw_load_sdcard(
	...
	if (FR_OK != res) {
		DLOG("firmware open failed: %s res=%d", path, (int)res);
	...
	result = _read_sd_payload(
		&file,
		path,
		file_size, //////boot_image->payload_size,
		progress_callback
	);
	if (FW_SUCCESS != result) {
		f_close(&file);
	...
	res = f_close(&file);
	if (FR_OK != res) {
		DLOG("firmware close failed: %s res=%d", path, (int)res);
		return FW_CLOSE_FAILED;
	}

	DLOG("Boot file bytes read: %u", (unsigned)boot_image->payload_size);
	return FW_SUCCESS;
...
static bool _read_exact(FIL *file, void *dest, UINT size) {
	...
	if (FR_OK != res) {
		DLOG("f_read failed: res=%d", (int)res);
	}
