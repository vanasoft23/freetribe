...
// Keep the bootloader MMU table in SRAM for factory firmware handoff.
__attribute__((section(".mmu_page_table"), aligned(16 * 1024)))
...
int main(void);
extern void boot_switch_to_ddr_stacks(void);
...
void start_boot(void) {
	...
	SysCfgRegistersLock(); // enable write-protection for registers of SYSCFG module
	
	SysCfgRegistersUnlock(); // disable write-protection for registers of SYSCFG module
	...
	boot_switch_to_ddr_stacks(); // stack was temporarily held in 8KiB ARM RAM, now we put it in DDR
...
static void _zero_bss(void) {
	...
	extern u32 _bss_end;
	for (u32 *ptr = &_bss_start; ptr < &_bss_end; ptr++) {
		*ptr = 0;
	}
}
...
static void _pll0_init(unsigned char clk_src, unsigned char pllm,
	...
	// TODO: KLOPT GEEN KANKER VAN.
	//       TRM CHAPTER 7.3, TABLE PLLCTL ZEGT:
	//        BIT 0 IS INTERNAL OSCILLATOR
	//        BIT 1 IS SQUARE WAVE
...
static void _config_cache_mmu(void) {
	...
	// DDR identity mapped and cached from 0xC0000000-0xC3F00000
