...
static void debug_print_frame(const volatile arm_frame_t *frame);
static const char *debug_filename(const char *path);
...
inline static void debug_print_sh(const char *str) {
    // An SWI taken from SVC mode overwrites LR_svc. Preserve the function's
    // return address explicitly so this helper is safe when not inlined.
    asm volatile (
        "stmdb sp!, {r4, lr}\n"
        "mov r0, %0\n"
        "mov r1, %1\n"
...
void fatal_error_impl(const char *format, ...) {
    ...
    debug_print_sh(s_str);
    debug_print_sh("\n");
    debug_print_frame(&g_fatal_frame);
