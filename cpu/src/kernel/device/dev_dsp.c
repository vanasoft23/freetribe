...
void dev_dsp_spi_tx_enqueue(u8 *p_byte) {
	...
	ring_buffer_put_overwrite(&dsp_spi_tx_rbd, p_byte, NULL);
...
bool dev_dsp_spi_rx_dequeue(u8 *p_byte) {

	return RING_BUFFER_OK == ring_buffer_get(&dsp_spi_rx_rbd, p_byte);
}
...
static void _dsp_spi_init(void) {
	...
	if (ring_buffer_init(&dsp_spi_tx_rbd, _ddr_uncached_alias(dsp_spi_tx_rbmem),
					 sizeof(dsp_spi_tx_rbmem[0]), ARRAY_SIZE(dsp_spi_tx_rbmem)) == RING_BUFFER_OK &&
		ring_buffer_init(&dsp_spi_rx_rbd, _ddr_uncached_alias(dsp_spi_rx_rbmem),
					 sizeof(dsp_spi_rx_rbmem[0]), ARRAY_SIZE(dsp_spi_rx_rbmem)) == RING_BUFFER_OK) {
...
static void _dsp_spi_rx_enqueue(u8 *p_byte) {
	...
	// Overwrite on overflow?
	ring_buffer_put_overwrite(&dsp_spi_rx_rbd, p_byte, NULL);
}
