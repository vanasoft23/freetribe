 ...
 * 
 * -- Expected transfer sequence per block --
 * 1. The DSP requests a host read transfer by raising the HSHK bit.
 * 2. EMIFA driver notices HSHK bit, and configures HostDMA using descriptor.
 * 3. HostDMA moves data words from DSP memory into FIFO.
 * 4. HostDP asserts HRDY (HOST_ACK pin low) to tell host FIFO is full.
 * 5. EMIFA reads data words from HostDP data port.
 * 6. HostDP drives HOST_ACK pin back up again once FIFO is empty.
 * 7. HostDMA fires HOSTRD_DONE IRQ with raised HOSTRD_DONE flag.
 * 8. HOSTRD_DONE ISR sees HOSTRD_DONE; modifies driver state, raises DMA_CMPLT.
 * 
 * -- Error scenario #1: DMA error occurs --
